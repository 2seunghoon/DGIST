...
class Trainer:
    ...
    def train_task(self, imgs, labels,coco_image,coco_gt):  # Train Task Networks (T)
        ...
        *_, GTA_feature = self.nets['T'](imgs[self.source], lbl=labels[self.source])
        ...
        fd_loss=0.
        try:
            for k in range(coco_image['G'].shape[0]):
                coco_im_baby = torch.cat([coco_image['G'][k,i].unsqueeze(0) for i in range(coco_image['G'][k,:].shape[0]) if coco_image['G'][k,i].sum().data.item()!=0.0],dim=0)
                coco_gt_baby = torch.cat([coco_gt['G'][k,i].unsqueeze(0) for i in range(coco_gt['G'][k,:].shape[0]) if coco_gt['G'][k,i].sum().data.item()!=0.0],dim=0)
                coco_gt_baby[coco_gt_baby==0] = -1

                *_, coco_feature = self.nets['T'](coco_im_baby, lbl=coco_gt_baby)
                coco_loss += self.nets['T'].loss_seg
                # coral_loss
                fd_loss = self.loss_fns.feature_distance3(GTA_feature, coco_feature, labels[self.source], self.ImageNet_classes,coco_gt_baby) 
                #
            self.losses['coco_seg_loss'] = coco_loss.data.item()
            self.losses['fd_loss'] = fd_loss
            ...
            coco_loss=0.
            fd_loss=0.
            self.losses['coco_seg_loss'] = 0.0
            self.losses['fd_loss'] = 0.0
            coco_im_baby=1
            coco_gt_baby=1

            # time.sleep(1)
        
        loss_task = loss_seg_src_origin+coco_loss+fd_loss
        loss_task.backward()
        self.optims['T'].step()
        self.losses['T'] = loss_seg_src_origin.data.item()
        return coco_im_baby,coco_gt_baby
        # self.losses['coco_seg_loss'] = coco_loss.data.item()
    ...
    def print_loss(self):
        self.logger.info(
            '[%d/%d] Origin: %.2f|cocoloss : %.2f|fd_loss : %.2f| %.2f %s'
            % (self.step, self.opt.iter,
                self.losses['T'],self.losses['coco_seg_loss'], self.losses['fd_loss'],self.best_miou, self.opt.ex))
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            tt,kk=self.train_task(imgs, labels,coco_image,coco_gt)
            if self.step % 1== 0:
