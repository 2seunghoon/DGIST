        ...
        try:
            for k in range(coco_image['G'].shape[0]):
                ...
                *_, coco_feature = self.nets['T'](coco_im_baby, lbl=coco_gt_baby)
                ...
                # coral_loss
                coral_loss = self.loss_fns.feature_distance(GTA_feature, coco_feature, labels[self.source], self.ImageNet_classes,coco_gt_baby) 

            self.losses['coco_seg_loss'] = coco_loss.data.item()
            self.losses['coral_loss'] = coral_loss
            ...
            self.losses['coral_loss'] = 0.0
            coco_im_baby=1
            coco_gt_baby=1

            # time.sleep(1)
        
        loss_task = loss_seg_src_origin+coco_loss+coral_loss
        loss_task.backward()
        self.optims['T'].step()
