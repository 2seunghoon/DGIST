...
class G_model(Cityscapes):
    ...
    def __getitem__(self, item):
        ...
        coco_img_dict, coco_gt_dict = self.get_coco_image(gt_image_ori)
        # image, gt_image  = self.copy_paste(image_ori, gt_image_ori, item)

        # image.save("image.png")
        # gt_image.save("gt_image.png")

        if (self.split == "train" or self.split == "trainval" or self.split =="all") and self.train:
            image, gt_image = self._train_sync_transform(image, gt_image_ori)
            coco_image_torch = {}
            coco_gt_torch = {}
            for i in coco_img_dict:
                coco_image_torch[i], coco_gt_torch[i] =  self._train_sync_transform(coco_img_dict[i], coco_gt_dict[i])
                # coco_image_torch_list.append(coco_image_torch)
                # coco_gt_torch_list.append(coco_gt_torch)
        else:
            image, gt_image = self._val_sync_transform(image, gt_image)
        coco_image=torch.zeros((34,3,256,512))
        coco_gt=torch.zeros((34,256,512))
        for i in coco_image_torch.keys():
            coco_image[i]+=coco_image_torch[i]
            coco_gt[i]+=coco_gt_torch[i]
        
        return image, gt_image, coco_image,coco_gt



    def get_coco_image(self, gt_image_ori):
        gt_number_list = np.unique(np.array(gt_image_ori))
        coco_img_dict = {}
        coco_gt_dict = {}
        for i in self.mask_labels:
            if i in gt_number_list:
                coco_img_dict[i], coco_gt_dict[i] = self.get_coco(i)
        return coco_img_dict, coco_gt_dict
    ...
    def copy_paste(self, image_ori, gt_image_ori, index_num):
        ...
        return self.output_img, self.output_gt

    def get_coco(self, class_num):
        # class random 하게 고르기
        while True:
            try:
                self.class_number = class_num # random.choice([19,24,26,27,28,31,32,33])
        ...
        return self.coco_img_pil, self.coco_mask_pil_class
    ...
    def place_object(self):
        ...
        pil_img = Image.fromarray(color_converted_img)
