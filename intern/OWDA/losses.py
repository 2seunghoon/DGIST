...
class Losses():
    ...
    def feature_distance(self, x1, x2, seg, classes, coco_gt,n_class=19):
        # x2=x2.unsqueeze(0)
        coral_loss = 0.
        ...
        cocomap=F.one_hot(coco_gt+1, num_classes=n_class+1).permute(0,3,1,2)
        cocomap = F.interpolate(cocomap.float(), size=x2.size()[2:], mode='nearest')
        x2=x2.unsqueeze(0)
        b_size = x1.shape[0]
        # h_size = x1.shape[2]
        # w_size = x1.shape[3]
        ...
        for i in range(b_size):
            for j in range(s_size):
                ...
                if j in coco_gt.unique()+1 and j!=0:
                    cnt+=1
                    if component_mask_area > 0:
                        cocomask=torch.sum(cocomap.bool()[cnt, j])
                        if cocomask >0:
                            # codes_component_feature1 = x1[i].masked_select(segmap.bool()[i, j]).reshape(f_size,  component_mask_area).mean(1)
                            codes_component_feature1 = x1[i].masked_select(segmap.bool()[i, j]).reshape(f_size,  component_mask_area)
                            codes_component_feature1=codes_component_feature1.t()
                            ...
                            codes_component_feature2 = x2[i][cnt].masked_select(cocomap.bool()[cnt, j]).reshape(f_size,  cocomask)
                            codes_component_feature2=codes_component_feature2.t()
                            # import pdb;pdb.set_trace()

                            # x2_centroid[i][j] = codes_component_feature2
                            coral_loss+=self.coral_loss(codes_component_feature1,codes_component_feature2)
                                   

        # x1_centroid = x1_centroid.mean(dim=0)[1:]
        # x1_centroid=x1_centroid[:,1:]
        # x2_centroid=x2_centroid[:,1:]
        ...
        # for c in classes:
            ...
            # coral_loss+=self.mmd_loss(x2_centroid[c],x1_centroid[c])
            # coral_loss+=self.coral_loss(x1_centroid[:,c,:],x2_centroid[:,c,:])


        return coral_loss
    def feature_distance2(self, x1, x2, seg, classes, n_class=19):
        fd_loss = 0.
        segmap = F.one_hot(seg+1, num_classes=n_class+1).permute(0,3,1,2)
        segmap = F.interpolate(segmap.float(), size=x1.size()[2:], mode='nearest')
        ...
        # x1_centroid = torch.zeros((b_size, s_size, f_size), dtype=x1.dtype, device=x1.device)
        # x2_centroid = torch.zeros((b_size, s_size, f_size), dtype=x2.dtype, device=x2.device)
        coral_loss=0.
        for i in range(b_size):
            for j in range(s_size):
                component_mask_area = torch.sum(segmap.bool()[i, j])
                if component_mask_area > 0 and j!=0:
                    codes_component_feature1 = x1[i].masked_select(segmap.bool()[i, j]).reshape(f_size,  component_mask_area)
                    codes_component_feature1=codes_component_feature1.t()
                    # x1_centroid[i][j] = codes_component_feature1
                    codes_component_feature2 = x2[i].masked_select(segmap.bool()[i, j]).reshape(f_size,  component_mask_area)
                    codes_component_feature2=codes_component_feature2.t()
                    coral_loss+=self.coral_loss(codes_component_feature1,codes_component_feature2)
                    # x2_centroid[i][j] = codes_component_feature2
        # x1_centroid = x1_centroid.mean(dim=0)[1:]
        ...
        # x2_centroid=x2_centroid[:,1:]
        # for c in classes:
            # fd_loss += self.coral_loss(x1_centroid[c],x2_centroid[c])
        return coral_loss
        
    def feature_distance3(self, x1, x2, seg, classes, coco_gt,n_class=19):
    # x2=x2.unsqueeze(0)
        # coral_loss = 0.
        segmap = F.one_hot(seg+1, num_classes=n_class+1).permute(0,3,1,2)
        segmap = F.interpolate(segmap.float(), size=x1.size()[2:], mode='nearest') # [1,20,512,1024] size에서 -> [1,20,65,129]로 변경, 20은 class 수
                ...
                if j in coco_gt.unique()+1 and j!=0:
                    ...
                    if component_mask_area > 0:
                        ...
                        if cocomask >0:
                            ...
                            x1_centroid[i][j] = codes_component_feature1
                            # codes_component_feature2 = x2[i].masked_select(segmap.bool()[i, j]).reshape(f_size,  component_mask_area).mean(1)
                            codes_component_feature2 = x2[i][cnt].masked_select(cocomap.bool()[cnt, j]).reshape(f_size,  cocomask).mean(1)
                            # codes_component_feature2=codes_component_feature2.t()
                            # import pdb;pdb.set_trace()

                            x2_centroid[i][j] = codes_component_feature2
                            # fd_loss+=F.l1_loss(codes_component_feature1,codes_component_feature2)
        ...
        return fd_loss
    def get_centroid_gta(self, x1, seg, classes,n_class=19):
