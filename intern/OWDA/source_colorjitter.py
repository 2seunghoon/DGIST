...
class Trainer:
    ...
    def tensor_board_log(self, imgs, labels,transfer,imgnet):
        nrow = 3
        ##
        ##
        ...
        for dset in self.opt.datasets:
            if dset=='G':
                ...
                self.writer.add_image('1_Input_Images/%s' % 'G', x, self.step)
                x = vutils.make_grid(imgs[dset].detach(), normalize=True, scale_each=True, nrow=nrow)
                self.writer.add_image('1_Input_Images/%s' % 'Source_Jitter', x, self.step)
            else:
                x = vutils.make_grid(imgs[dset].detach(), normalize=True, scale_each=True, nrow=nrow)
        ...
        for dset in self.opt.datasets:
            ...
            if dset=='G':
                imgnet_preds[dset]=self.nets['T'](imgnet[dset])[0]
        
        
        ## for transfer
        key='G'
        imgnet_pred = imgnet_preds[key].data.cpu().numpy()
        imgnet_pred = np.argmax(imgnet_pred, axis=1)
        x = decode_labels(imgnet_pred, num_images=self.opt.batch)
        x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
        self.writer.add_image('5_Prediction/%s' % 'G', x, self.step)        
        #####
        for key in preds.keys():
            if key=='G':
                ...
                x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
                self.writer.add_image('5_Prediction/%s' % 'Source_Jitter', x, self.step)
            else:
                pred = preds[key].data.cpu().numpy()
                pred = np.argmax(pred, axis=1)
                x = decode_labels(pred, num_images=self.opt.batch)
                x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
                self.writer.add_image('5_Prediction/%s' % dset, x, self.step)
    ...
    def print_loss(self):
        self.logger.info(
            '[%d/%d] Origin ColorJitter: %.2f| %.2f %s'
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            for dset in self.opt.datasets:
                if dset=='G':
                    imgs[dset], labels[dset],transfer[dset],imgnet[dset]= batch_data[dset] ## imgnet : original input GTA5 Image
                    imgs[dset], labels[dset],transfer[dset] ,imgnet[dset]= imgs[dset].cuda(), labels[dset].cuda(),transfer[dset].cuda(),imgnet[dset].cuda()
            ...
            if self.step % 10 == 0:
                self.tensor_board_log(imgs, labels,transfer,imgnet)
