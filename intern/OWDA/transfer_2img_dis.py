...
class Trainer:
    ...
    def set_optimizers(self):
        ...
        self.optims['D']=optim.SGD(self.discriminator.parameters(),lr=1e-3,momentum=0.9)
    ...
    def train_task2(self, imgs, labels,coco_img,coco_gt):  
        # Train Discrim
        ...
        for _ in range(1):
            ...
            *_, GTA_feature = self.nets['T'](imgs[self.source], lbl=labels[self.source])
            *_, transfer_feature = self.nets['T'](coco_img[self.source], lbl=coco_gt[self.source])
...
##
            ...
            discriminator_y = torch.cat([torch.zeros((GTA_feature.shape[0],1,8,16), device=device),torch.ones((transfer_feature.shape[0],1,8,16), device=device)])
            loss_D = self.criterion(preds, discriminator_y)
            
            ##
            # self.optims['T'].zero_grad()

            loss_task = loss_seg_src_origin+loss_D
            loss_task.backward()
            self.optims['T'].step()
            self.losses['T'] = loss_seg_src_origin.data.item()
            # self.losses['synthesis'] = loss_seg_src_synthesis.data.item()
    ...
    def print_loss(self):
        self.logger.info(
            '[%d/%d] Origin: %.2f|MappingLoss : %.2f|Discriminator : %.2f| %.2f %s'
            % (self.step, self.opt.iter,
                self.losses['T'],self.losses['E'],self.losses['D'], self.best_miou, self.opt.ex))
        ...
        for i in range(self.opt.iter):
            ...
            for dset in self.opt.datasets:
                if dset=='G':
                    ...
                    imgs[dset], labels[dset],coco_img[dset],coco_gt[dset]= batch_data[dset]
                    imgs[dset], labels[dset],coco_img[dset],coco_gt[dset] = imgs[dset].cuda(), labels[dset].cuda(),coco_img[dset].cuda(),coco_gt[dset].cuda()
            ...
            self.train_task2(imgs, labels,coco_img,coco_gt)
            # if self.step % 10 == 0:
