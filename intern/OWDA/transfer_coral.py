...
class Trainer:
    ...
    def train_task(self, imgs, labels,transfer):  # Train Task Networks (T)
        ...
        *_, transfer_feature = self.nets['T'](transfer[self.source], lbl=labels[self.source])
        ...
        coralLoss = self.loss_fns.feature_distance2(GTA_feature, transfer_feature, labels[self.source], self.ImageNet_classes) 
        loss_task = loss_seg_src_origin+loss_seg_src_synthesis+coralLoss
        loss_task.backward()
        self.optims['T'].step()
        self.losses['T'] = loss_seg_src_origin.data.item()
        self.losses['synthesis'] = loss_seg_src_synthesis.data.item()
        self.losses['coral']=coralLoss
    ...
    def print_loss(self):
        self.logger.info(
            '[%d/%d] Origin: %.2f|Synthesis : %.2f|Coral : %.2f| %.2f %s'
            % (self.step, self.opt.iter,
                self.losses['T'],self.losses['synthesis'],self.losses['coral'], self.best_miou, self.opt.ex))
