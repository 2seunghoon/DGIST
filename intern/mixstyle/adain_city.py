...
class Trainer:
    ...
    def train_task(self, imgs, labels,imgnet):  # Train Task Networks (T)
        ...
        *_, GTA_feature = self.nets['T'](transfer, lbl=labels[self.source])
        ...
        loss_task = loss_seg_src_synthesis
        loss_task.backward()
        self.optims['T'].step()
    ...
    def tensor_board_log(self, imgs, labels,imgnet,transfer):
        ...
        self.writer.add_image('1_Input_Images/%s' % 'Transfer_GTA5', x, self.step)
        ...
        with torch.no_grad():
            _,_,transfer_city=self.network(imgs[self.target],imgnet[self.source])
            transfer_city=transfer_city.detach()
            x = vutils.make_grid(transfer_city.detach(), normalize=True, scale_each=True, nrow=nrow)
            self.writer.add_image('1_Input_Images/%s' % 'Transfer_CityScape', x, self.step)
        ##     ##
        ...
        with torch.no_grad():
            ...
            for dset in self.opt.datasets:
                ...
                if dset=='C':
                    trans_preds_city[dset]=self.nets['T'](transfer_city)[0].data.cpu().numpy()
                if dset=='G':
            ...
            x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
            self.writer.add_image('5_Prediction/%s' % 'Transfer_GTA5', x, self.step)        
            #####
            key='C'
            trans_pred_city = trans_preds_city[key]
            trans_pred_city = np.argmax(trans_pred_city, axis=1)
            x = decode_labels(trans_pred_city, num_images=self.opt.batch)
            x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
            self.writer.add_image('5_Prediction/%s' % 'Transfer_CityScapre', x, self.step)    
    ...
    def eval(self, target,imgnet_buffer):
        self.set_eval()
        ...
        with torch.no_grad():
            for batch_idx, (imgs, labels) in enumerate(self.test_loader[target]):
                # imgs['G'], labels['G'],imgnet['G']=batch_data['G']
                imgs, labels = imgs.cuda(), labels.cuda()
                labels = labels.long()

                ## transfer
                ## random choice in buffer
                ran=random.randrange(0,len(imgnet_buffer))
                choiced=imgnet_buffer[ran].cuda()
                _,_,transfer_city=self.network(imgs,choiced)
                transfer_city=transfer_city.detach()
                pred, *_ = self.nets['T'](transfer_city)
                ##
                # pred, *_ = self.nets['T'](imgs) 원본 코드
    ...
    def print_loss(self):
        self.logger.info(
            '[%d/%d] Synthesis : %.2f|Content : %.2f|Style : %.2f|%.2f %s'
            % (self.step, self.opt.iter,
                self.losses['synthesis'],self.losses['content'],self.losses['style'],self.best_miou, self.opt.ex))
        ...
        for i in range(self.opt.iter):
            ...
            if self.step % self.opt.eval_freq == 0:
                for target in self.targets:
                    self.eval(target,imgnet_buffer)
            self.print_loss()
