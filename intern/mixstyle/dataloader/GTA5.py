...
class GTA5(Cityscapes):
    ...
    def __getitem__(self, item):
        ...
        image_imgnet=Image.open(image_path_imgnet).convert("RGB")
        # edge=cv2.Canny(image_cv,100,255)
        # edge=cv2.GaussianBlur(image_cv,(3,3),0)
        # edge_gray=cv2.cvtColor(edge,cv2.COLOR_BGR2GRAY)
        # edge = cv2.Laplacian(edge_gray, cv2.CV_8U, ksize=3)
        ...
        # transfer = color_transfer(image_imgnet, image_cv)
        ...
        if (self.split == "train" or self.split == "trainval" or self.split =="all") and self.train:
            ...
            # edge=self._train_sync_transform(edge, None)
        ## 여기서 synthesis이미지도 transform해줄거
            ...
            # edge=self._val_sync_transform(edge, None)
        ...
        return image, gt_image,image_imgnet
