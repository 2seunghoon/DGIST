...
class MixStyle(nn.Module):
    ...
    def forward(self, x):
        ...
        perm = torch.arange(batch_size - 1, -1, -1)  # inverse index 거꾸로하기
        perm_b, perm_a = perm.chunk(2)
        perm_b = perm_b[torch.randperm(batch_size // 2)]
        perm_a = perm_a[torch.randperm(batch_size // 2)] # perm random하게 섞음
...
class ResNet101(nn.Module):
    def __init__(self, block, layers, num_classes, phase):
        ...
        # Mix style
        # self.mixStyleModule=MixStyle(p=1,alpha=0.1)
    ...
    def forward(self, x, lbl=None, weight=None, ita=1.5, from_layer1 = False, predicted_weight=None):
        feature = None
        # features = []
        if from_layer1:
            h, w = 512, 1024
        ...
        if self.phase == 'train':
            ...
            if predicted_weight is not None:
                predicted_weight_ = nn.functional.interpolate(predicted_weight, size=(h, w), mode='bilinear', align_corners=True)
                x = predicted_weight_ * x
            if lbl is not None:
                self.loss_seg = self.CrossEntropy2d(x, lbl, weight=weight)

        return x, pred, feature
