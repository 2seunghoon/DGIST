...
class Trainer:
    ...
    def train_task(self, imgs, labels,imgnet,edge):  # Train Task Networks (T)
        self.adjust_learning_rate(self.optims['A'],self.step-1)
        self.set_zero_grad()
        # for adain
        loss_c,loss_s,transfer=self.network(edge[self.source],imgnet[self.source])
        loss_s=10*loss_s
        ...
        for i in range(self.opt.iter):
            ...
            for dset in self.opt.datasets:
                if dset=='G':
                    ...
                    imgs[dset], labels[dset],imgnet[dset],edge[dset] = imgs[dset].cuda(), labels[dset].cuda(),imgnet[dset].cuda(),edge[dset].cuda()
            ...
            transfer=self.train_task(imgs, labels,imgnet,edge)
            if self.step % 1 == 0:
