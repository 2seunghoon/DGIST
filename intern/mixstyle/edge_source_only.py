...
class Trainer:
    ...
    def train_task(self, imgs, labels,edge):  # Train Task Networks (T)

        self.set_zero_grad()
        *_, GTA_feature = self.nets['T'](edge[self.source], lbl=labels[self.source])
    ...
    def tensor_board_log(self, imgs, labels,edge):
        nrow = 5
        ##
        ##
        ...
        ## transfer
        x = vutils.make_grid(edge['G'].detach(), normalize=True, scale_each=True, nrow=nrow)
        self.writer.add_image('1_Input_Images/%s' % 'edge', x, self.step)
        ...
        for dset in self.opt.datasets:
            ...
            if dset=='G':
                trans_preds[dset]=self.nets['T'](edge[dset])[0]
        ...
        x = vutils.make_grid(x, normalize=True, scale_each=True, nrow=nrow)
        self.writer.add_image('5_Prediction/%s' % 'edge', x, self.step)        
        #####
        ...
        for i in range(self.opt.iter):
            ...
            for dset in self.opt.datasets:
                if dset=='G':
                    ...
                    imgs[dset], labels[dset],imgnet[dset],edge[dset]= batch_data[dset]
                    imgs[dset], labels[dset],edge[dset] = imgs[dset].cuda(), labels[dset].cuda(),edge[dset].cuda()
            ...
            self.train_task(imgs, labels,edge)
            if self.step % 1 == 0:
                self.tensor_board_log(imgs, labels,edge)
