...
class Trainer:
    ...
    def train_task(self, imgs, labels,imagenet_pil,gaussain_image,things_stuff,class_aug,color_jitter,c_transfer):  # Train Task Networks (T)
        self.adjust_learning_rate(self.optims['A'],self.step-1)
        self.set_zero_grad()
        # for adain
        loss_c,loss_s,transfer=self.network(imgs[self.source],imagenet_pil[self.source])
        loss_s=10*loss_s
        ...
        *_, GTA_feature = self.nets['T'](gaussain_image, lbl=labels[self.source])
        loss_seg_src_gaussian = self.nets['T'].loss_seg
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            for dset in self.opt.datasets:
                if dset=='G':
                    imgs[dset], labels[dset],imagenet_pil[dset],gaussain_image[dset],things_stuff[dset],class_aug[dset],color_jitter[dset],c_transfer[dset]= batch_data[dset]
                    imgs[dset], labels[dset],imagenet_pil[dset],gaussain_image[dset],things_stuff[dset],class_aug[dset],color_jitter[dset],c_transfer[dset] = imgs[dset].cuda(), labels[dset].cuda(),imagenet_pil[dset].cuda(),gaussain_image[dset].cuda(),things_stuff[dset].cuda(),class_aug[dset].cuda(),color_jitter[dset].cuda(),c_transfer[dset].cuda()
            ...
            transfer=self.train_task(imgs, labels,imagenet_pil,gaussain_image,things_stuff,class_aug,color_jitter,c_transfer)
            if self.step % 10 == 0:
